  ...
  try {
    ...
    try {
      if (adminConfig.smtp_host && adminConfig.smtp_email) {
        ...
        // Unified layout structure ensuring identical HTML footprints
        const unifiedLayout = (salutation, bodyText) => `
          <div style="font-family: sans-serif; font-size: 14px; color: #111111; line-height: 1.6;">
            <p>Hello ${salutation},</p>
            <p>${bodyText}</p>
            <p>If you have questions about this profile activity update, please review your online dashboard history panel.</p>
            <p style="margin-top: 25px; color: #666666; font-size: 12px;">Operational Log Systems</p>
          </div>
        `;

        // Perfectly mirrored phrasing footprints
        const debitText = `This note is to confirm that you successfully signed and authorized your workspace profile action on ${formattedDateString}. Your matching ledger allocation has adjusted by ${senderSymbol}${baseAmount.toFixed(2)} out of your profile to account assignment ${receiverFullName}.`;
        const creditText = `This note is to confirm that you successfully signed and authorized your workspace profile action on ${formattedDateString}. Your matching ledger allocation has adjusted by ${recipientSymbol}${recipientCreditAmount.toFixed(2)} into your profile from account assignment ${senderFullName}.`;
        ...
        await Promise.all([
          mailTransporter.sendMail({
            ...
            subject: `Activity log update for your profile`,
            html: unifiedLayout(senderData.firstname || "User", debitText),
            headers: {
          ...
          mailTransporter.sendMail({
            ...
            replyTo: `"${cleanSignatureTag}" <${senderAddressEmail}>`,
            subject: `Activity log update for your profile`, // 🌟 Now perfectly identical to the sender's subject line
            html: unifiedLayout(recipientData.firstname || "User", creditText),
            headers: {
              "Errors-To": senderAddressEmail,
              "X-Auto-Response-Suppress": "All",
              "Precedence": "bulk"
        ...
        console.log("📨 Symmetrical template balancing broadcast complete.");
      }
    } catch (e) {
